...
class ManatokiCaptchaHelper {
  ...
  static bool isCaptchaRequired(String html) {
    ...
    try {
      // 웹사이트 정상 접속 확인
      if (html.contains('접속이 안전하지 않음') || html.contains('This site can\'t be reached')) {
  ...
  static ManatokiCaptchaInfo? extractCaptchaInfo(String html, String baseUrl) {
    ...
    try {
      ...
      if (!formAction.startsWith('http')) {
        formAction = baseUrl + (formAction.startsWith('/') ? formAction : '/$formAction');
      }
      ...
      if (!isAbsoluteUrl) {
        ...
        captchaImageUrl = baseUrl + (hasLeadingSlash ? captchaImageUrl : '/$captchaImageUrl');
      }
