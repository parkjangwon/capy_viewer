...
MangaDetailParseResult parseMangaDetailFromHtml(String html, String mangaId) {
  ...
  // 제목
  String title = document.querySelector('.view-content span[style*="font-size:20px"] b')?.text.trim() ?? '';
  ...
  // 썸네일
  String thumbnailUrl = document.querySelector('.view-img img')?.attributes['src'] ?? '';
  ...
  for (final item in items) {
    ...
    int comments = 0;
    // 댓글 수 추출 및 span 제거
    aTag?.querySelectorAll('.count, .wr-comment, .orangered').forEach((e) => e.remove());
    ...
    final views = int.tryParse(item.querySelector('.wr-hit')?.text.replaceAll(',', '').trim() ?? '0') ?? 0;
    final likes = int.tryParse(item.querySelector('.wr-good')?.text.replaceAll(',', '').trim() ?? '0') ?? 0;
    final ratingText = item.querySelector('.wr-star')?.text ?? '';
    final ratingMatch = RegExp(r'\((\d+(?:\.\d+)?)\)').firstMatch(ratingText);
    final rating = ratingMatch != null ? double.tryParse(ratingMatch.group(1)!)?.toInt() ?? 0 : 0;

    // 전편보기 버튼 링크 추출
    String? fullViewUrl;
    final fullViewButton = item.querySelector('button[data-original-title="전편보기"]');
