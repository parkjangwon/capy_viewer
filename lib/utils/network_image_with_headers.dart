...
class NetworkImageWithHeaders extends ConsumerWidget {
  ...
  Future<Uint8List?> _fetchImageBytes(String siteBaseUrl, CookieJar cookieJar) async {
    try {
      ...
      bool isCaptchaImage = imageUrl.contains('captcha') || imageUrl.contains('kcaptcha');
      
      // 캡챠 이미지이고 PHPSESSID가 없는 경우 더미 추가
      if (isCaptchaImage && !hasPhpSessionId) {
        allCookies.add(Cookie('PHPSESSID', 'dummy_session_${DateTime.now().millisecondsSinceEpoch}'));
      }
      ...
      if (isCaptchaImage) {
        final browserCookies = [
          ...
          Cookie('cf_clearance', 'Smlpj2_ehK4z7yGnbr7P1B9rkj2OcJKcqfnJbwRwt-1746323245-0'),
      ...
      try {
        ...
        if (httpResponse.statusCode == 200 && httpResponse.bodyBytes.isNotEmpty) {
          ...
          print('HTTP 이미지 로드 실패: $urlWithTimestamp, 상태 코드: ${httpResponse.statusCode}');
          ...
          if (secondResponse.statusCode == 200 && secondResponse.bodyBytes.isNotEmpty) {
            ...
            print('두 번째 HTTP 이미지 로드 실패: $imageUrl, 상태 코드: ${secondResponse.statusCode}');
          }
        }
      } catch (httpError) {
      ...
      if (response.statusCode == 200 && response.data != null) {
        ...
        if (response.data!.length > 0) {
          ...
          final previewSize = response.data!.length > 20 ? 20 : response.data!.length;
          print('데이터 미리보기 (처음 $previewSize 바이트): ${response.data!.sublist(0, previewSize)}');
        }
      }
      ...
      if (response.statusCode == 200 && response.data != null && response.data!.isNotEmpty) {
        ...
        print('Dio 이미지 로드 실패: ${response.requestOptions.uri}, 상태 코드: ${response.statusCode}');
