...
class RecentAddedParser {
  ...
  static List<RecentAddedItem> parseRecentAddedList(String htmlText) {
    ...
    for (final el in postRows) {
      ...
      if (postTextEl != null) {
        ...
        if (tagIcons.length > 1) {
          ...
          if (idx >= 0 && idx + 1 < nodes.length) {
            ...
            genres = g.split(',').map((e) => e.trim()).where((e) => e.isNotEmpty).toList();
          }
        }
      }
      // 날짜
      ...
      // 조회수
      final viewsText = el.querySelector('.fa-eye')?.parent?.text.replaceAll(RegExp(r'[^0-9,]'), '').replaceAll(',', '') ?? '';
      final views = int.tryParse(viewsText);
      // 좋아요
      final likesText = el.querySelector('.fa-thumbs-o-up')?.parent?.text.replaceAll(RegExp(r'[^0-9]'), '') ?? '';
      final likes = int.tryParse(likesText);
      // 댓글
      final commentsText = el.querySelector('.fa-commenting-o')?.parent?.text.replaceAll(RegExp(r'[^0-9]'), '') ?? '';
