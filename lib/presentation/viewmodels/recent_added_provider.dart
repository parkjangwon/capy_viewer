...
/// 홈 화면: 최신 6개만
final recentAddedPreviewProvider =
    FutureProvider<List<RecentAddedItem>>((ref) async {
  ...
  final filteredItems = await Future.wait(
    items.map((item) async {
      ...
      results.where((item) => item != null).cast<RecentAddedItem>().toList());

  return filteredItems.take(6).toList();
});

/// 더보기(전체): 페이지네이션 (20개씩)
final recentAddedPagingProvider =
    FutureProvider.family<List<RecentAddedItem>, int>((ref, page) async {
  final api = ref.read(apiServiceProvider());
