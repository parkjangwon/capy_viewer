...
class CloudflareCaptchaHelper {
  ...
  static Future<void> processCaptcha(WebViewController controller) async {
    try {
      ...
      // 1. 캡차 페이지인지 확인
      final isCaptchaPageResult =
          await controller.runJavaScriptReturningResult(detectCaptchaScript);
      final isCaptchaPage =
          isCaptchaPageResult.toString().toLowerCase() == 'true';
  ...
  /// 페이지 쿠키 가져오기
  static Future<List<String>> getPageCookies(
