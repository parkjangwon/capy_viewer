        ...
        try {
          ...
          if (setCookieHeader != null && setCookieHeader.isNotEmpty) {
            ...
            if (setCookieHeader.contains('PHPSESSID=')) {
              int startIndex =
                  setCookieHeader.indexOf('PHPSESSID=') + 'PHPSESSID='.length;
              ...
              await cookieJar
                  .saveFromResponse(uri, [Cookie('PHPSESSID', phpSessionId)]);
  ...
  Future<Uint8List?> _fetchImageBytes(String? phpSessionId) async {
    try {
      ...
      if (!imageUrl.contains('?t=') && !imageUrl.contains('&t=')) {
        imageUrl =
            '$imageUrl${imageUrl.contains('?') ? '&' : '?'}t=$currentTimestamp';
      }
      ...
      for (var cookie in domainCookies) {
        ...
        if (cookie.name == 'PHPSESSID' &&
            allCookies.any((c) => c.name == 'PHPSESSID')) {
          continue;
        }
      ...
      if (!hasCfClearance) {
        allCookies.add(Cookie('cf_clearance',
      ...
      final urlWithoutParams =
          '${imageUrl.split('?')[0]}?t=$timestamp&_direct=1';
      final directUrl =
          '$baseUrl/plugin/kcaptcha/kcaptcha_image.php?t=$timestamp';
      ...
      for (int i = 0; i < urlVariations.length; i++) {
        ...
        try {
          ...
          final response =
              await http.get(Uri.parse(currentUrl), headers: headers);
              ...
              try {
                ...
                for (var cookieStr in cookies) {
                  ...
                  if (parts.length == 2) {
                    ...
                    await cookieJar
                        .saveFromResponse(uri, [Cookie(name, value)]);
